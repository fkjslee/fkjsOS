...
int strtol(char *s, char **endp, int base);	/* ïWèÄä÷êîÅistdlib.hÅj */
	...
	for (;;) {
		...
		if (i == '4') {
			for (;;) {
				...
				if (api_getkey(0) != '4') { /* î@â ñvóLà¬â∫4?óù?ë© */
					break;
				}
			}
		}
		...
		if (i == '8') {
			for (;;) {
				for (j = 0; j < spd_y; j++) {
					...
					for (p--; p[-1] != 0x0a; p--) { } /* âÒüËìûè„àÍò¢éöïÑ?0x0a?é~ */
				}
				if (api_getkey(0) != '8') {
					break;
				}
